...
void nearest_neighbor_serv::get_status(status_t& status) const {
  ...
  my_status["data"] = lexical_cast<string>(nearest_neighbor_->get_table()->dump_json());
...
void nearest_neighbor_serv::set_config(const std::string& config) {
  ...
  pfi::lang::shared_ptr<jubatus::core::nearest_neighbor::nearest_neighbor_base> nn(jubatus::core::nearest_neighbor::create_nearest_neighbor(conf.method, param, table, my_id));
  nearest_neighbor_.reset(new core::driver::nearest_neighbor(nn, table, converter));
