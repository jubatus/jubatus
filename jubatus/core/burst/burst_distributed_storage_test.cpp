...
namespace burst {
namespace storage {
// using namespace jubatus::core::burst::system;

/**
 * setup
 * $ sudo /usr/share/zookeeper/bin/zkServer.sh start
 */
TEST(burst_distributed_storage, general) {
  shared_ptr<burst_distributed_storage> b =
  ...
  vector<string> nodes_vec1;
  nodes_vec1.push_back(string("192.51.52.132_9180"));
  nodes_vec1.push_back(string("192.51.52.133_9180"));
  nodes_vec1.push_back(string("192.51.52.134_9180"));
  b->update_cht_local_nodes(nodes_vec1);
  ...
  /*
   * my_keyword: keyword3 only
   */
  ASSERT_FALSE(b->is_my_keyword("keyword01"));
  ASSERT_FALSE(b->is_my_keyword("keyword02"));
  ASSERT_TRUE(b->is_my_keyword("keyword03"));
  ASSERT_FALSE(b->is_my_keyword("keyword04"));
  ...
  ASSERT_EQ(1, static_cast<int>(bursted_results.size()));
  map<string, jubatus::st_window>::iterator it = bursted_results.begin();
