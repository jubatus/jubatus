...
let gen_impl names s =
  ...
  concat_blocks [
    ...
    [
      ...
      (2,   "rpc_server::add<std::map<std::string, std::string>(std::string, std::string)>(\"save\", jubatus::util::lang::bind(&"
    ...
    [
      (1,   "std::map<std::string, std::string> save(const std::string& id) {");
      (2,     "JRLOCK_(p_);");
