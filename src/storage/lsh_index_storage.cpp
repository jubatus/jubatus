...
void retrieve_hit_rows_from_table(size_t hash, const lsh_table_t& table, unordered_set<uint64_t>& cands) {
...
lsh_master_table_t::iterator lsh_index_storage::remove_and_get_row(const string& row) {
  ...
  for (size_t i = 0; i < entry.lsh_hash.size(); ++i) {
    ...
    if (it != lsh_table_diff_.end()) {
      ...
      vector<uint64_t>::iterator jt = lower_bound(range.begin(), range.end(), row_id);
