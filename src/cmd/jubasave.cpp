...
static const std::string PROGNAME(JUBATUS_APPNAME "save");
...
bool callsave(const string&, const int, const string&, const string&);
...
int main(int args, char** argv) try {
  ...
  if (iprevert(snpn, server, port)){
    if(callsave(server, port, name, model)){
    std::cout << "save " << model << " successfully" << std::endl;
   }
  }
...
bool callsave(const string& server,const int port, const string& name, const string& model){
    
    commonrpc crpc(server, port, 10);
    return crpc.save(name, model);
}
