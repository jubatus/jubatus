...
public:
  ...
  rpc_mclient(const std::vector<std::pair<std::string, int> >& hosts, int timeout_sec,
    ...
    for(size_t i = 0; i < hosts.size(); ++i) hosts_.push_back( hosts[i] );
...
private:
  ...
  rpc_result<Res> join_(const std::string &method, const pfi::lang::function<Res(Res,Res)> &reducer );
  template<typename Res>
  void join_one_(const std::string &method, msgpack::rpc::future &response, rpc_result<Res>& result ,
...
rpc_result<Res> rpc_mclient::join_(const std::string &method, const pfi::lang::function<Res(Res,Res)> &reducer ) {
  ...
  for(size_t i = 0; i < futures_.size(); ++i) {
    try {
      ...
      result.error.push_back( rpc_error( hosts_[i].first, hosts_[i].second, 
  ...
  if ( result_count == 0 ) {
    rpc_no_result e;
    if ( result.has_error() ) e << error_multi_rpc(result.error);
