...
{
  pfi::lang::function<void(int,int,string)>* fp = static_cast<pfi::lang::function<void(int,int,string)>*>(watcherCtx);
  (*fp)(type, state, string(path));
  delete fp;
}

bool zk::bind_watcher(const string& path, pfi::lang::function<void(int,int,string)>& f)
{
  pfi::lang::function<void(int,int,string)> * fp = new pfi::lang::function<void(int,int,string)>(f);
