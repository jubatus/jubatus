...
TEST(pb_string_feature, trivial) {
  // Test fixture setup.
  initialize();
  add_path(".");
  ...
  params["class"] = "StringFeature";
  pb_string_feature pb(setup(params));

  vector<string_feature_element> result;
  pb.extract("my_text", result);
  ASSERT_EQ(1, result.size());
  EXPECT_EQ(0, result[0].begin);
  EXPECT_EQ(0, result[0].length);
  EXPECT_EQ("test", result[0].value);
